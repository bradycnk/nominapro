...
const DashboardOverview: React.FC<DashboardOverviewProps> = ({ config, totalEmployees, estimatedPayrollVEF, setActiveTab }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data: attendances, error: attError } = await supabase.from('asistencias').select('*').gte('fecha', firstDayOfMonth).lte('fecha', lastDayOfMonthISO);
      if (attError) return;
      ...
      const performance = employees.map(emp => {
        const empAttendances = attendances?.filter(a => a.empleado_id === emp.id) || [];
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-6 duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
        ...
        <div className="lg:col-span-2 bg-white p-10 rounded-[3rem] shadow-xl border border-slate-100 relative overflow-hidden group">
          ...
          <p className="text-slate-500 leading-relaxed text-lg mb-10 max-w-xl">
            ...
            {attendanceToday.reported 
              ? ` Hoy se han registrado ${attendanceToday.present} de ${attendanceToday.total} asistencias.` 
              : " Aún no se han cerrado registros de asistencia para el día de hoy."}
          </p>
        ...
        <div className="bg-[#1E1E2D] p-8 rounded-[3rem] shadow-2xl flex flex-col justify-between text-white relative overflow-hidden group">
            ...
            <div>
              ...
              <h3 className="text-xl font-black mb-2 tracking-tight">Próximos Eventos</h3>
              <p className="text-slate-400 text-xs font-medium leading-relaxed">
                {pendingLoans > 0 
                  ? `Tienes ${pendingLoans} solicitud(es) de adelanto pendientes de aprobación.` 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
        {topEmployee && <EmployeePerformanceCard employee={topEmployee.employee} stats={topEmployee.stats} rank="High" />}
        {bottomEmployee && <EmployeePerformanceCard employee={bottomEmployee.employee} stats={bottomEmployee.stats} rank="Low" />}
      </div>
