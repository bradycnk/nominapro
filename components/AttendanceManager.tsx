...
const AttendanceManager: React.FC = () => {
  ...
  const deleteDay = async () => {
    const recordToDelete = selectedDate ? getHistoryRecordByDate(selectedDate) : null;
    ...
    try {
      ...
      setDayDraft({ estado: 'presente', hora_entrada: '', hora_salida: '', observaciones: '' });
  ...
  const saveCalendarDay = async () => {
    ...
    try {
      ...
      // Nueva Lógica: Un solo registro con fecha y hora completa
      // Si el turno cruza medianoche, simplemente la hora_salida será del día siguiente.
      const payload = {
        empleado_id: selectedEmployeeId,
        fecha: selectedDate, // Mantenemos 'fecha' como la fecha de inicio del turno
      ...
      // Si ya existe un registro para esta fecha exacta (mismo inicio de turno), lo actualizamos por ID
              ...
              {record?.estado === 'presente' ? (
                <div className="flex flex-col gap-0.5 mt-auto">
                    ...
                    <div className="text-[9px] text-slate-500 font-mono mb-1">
                        {formatDisplayTime(record.hora_entrada)} - {formatDisplayTime(record.hora_salida)}
                    </div>
